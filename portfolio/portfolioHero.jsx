      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 font-sans px-8 py-12 md:px-24 max-w-7xl mx-auto">
        ...
        <div className="max-w-3xl space-y-8">
          ...
          <p className="text-lg text-slate-400 leading-relaxed">
            A full-stack web application that tracks ATP tennis player performance metrics and directly correlates live competitive statistics with real-world memorabilia valuation trends across digital marketplaces.
          </p>

          <div className="border-t border-b border-slate-800 py-6 my-8 grid grid-cols-2 md:grid-cols-4 gap-4 font-mono text-xs">
            <div><span className="text-slate-500 block">ROLE</span> Full-Stack Developer</div>
            <div><span className="text-slate-500 block">STACK</span> React, Node, Express</div>
            <div><span className="text-slate-500 block">DATA</span> eBay API, ATP CSV</div>
            <div><span className="text-slate-500 block">STATUS</span> Production Active</div>
          </div>
          ...
          <ul className="list-disc list-inside text-slate-400 space-y-2 text-sm leading-relaxed">
            <li>Dynamic lookups across prominent ATP professional configurations (Alcaraz, Sinner, Djokovic).</li>
            <li>Direct ingestion grids detailing surface trends, career win/loss metrics, and historical scores.</li>
            <li>Calculates live market performance indices (average, minimum, and maximum valuation metrics) utilizing authentic production APIs.</li>
          </ul>
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 font-mono text-xs text-slate-400 rounded space-y-1 overflow-x-auto">
            <p>User Browser → React Client (Port 5173)</p>
            <p className="pl-12">↓ Express REST Engine (Port 3001)</p>
            <p className="pl-24">├── eBay Browse API (Live Price Polling)</p>
            <p className="pl-24">└── Local ATP Datasets (Match History Grids)</p>
          </div>
        </div>
        <ProjectFooter />
      </div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 font-sans px-8 py-12 md:px-24 max-w-7xl mx-auto">
        ...
        <div className="max-w-3xl space-y-8">
          ...
          <p className="text-lg text-slate-400 leading-relaxed">
            An advanced financial crime investigation framework leveraging spatial network representations to parse complex money rings, structuring, and multi-layered assets down to isolated anomalies instantly.
          </p>

          <div className="border-t border-b border-slate-800 py-6 my-8 grid grid-cols-2 md:grid-cols-4 gap-4 font-mono text-xs">
            <div><span className="text-slate-500 block">ROLE</span> ML Systems Engineer</div>
            <div><span className="text-slate-500 block">ARCHITECTURE</span> 6-Layer Pipeline</div>
            <div><span className="text-slate-500 block">CORE GNN</span> GraphSAGE Model</div>
            <div><span className="text-slate-500 block">EVALUATION</span> 95% Graph Accuracy</div>
          </div>

          <h3 className="text-xl font-bold text-slate-200 pt-4">Technical Overview</h3>
          <p className="text-slate-400 text-sm leading-relaxed">
            Traditional transactional rules often miss relational structures across overlapping account nodes. By implementing a multi-layered topological evaluation process, the pipeline detects suspicious patterns and reduces investigation cycles from hours to under 60 seconds.
          </p>

          <h3 className="text-xl font-bold text-slate-200 pt-4">System Breakdown</h3>
          <ul className="list-disc list-inside text-slate-400 space-y-2 text-sm leading-relaxed">
            <li><strong className="text-slate-200">PyTorch Geometric Ensembles:</strong> GraphSAGE layer passes node messages over transaction graphs to catch hidden structures.</li>
            <li><strong className="text-slate-200">Regulatory Automation:</strong> Integrates Gemini LLM to auto-generate regulatory-grade Suspicious Activity Reports (SAR) based on data matrices.</li>
            <li><strong className="text-slate-200">Contextual Ingestion:</strong> Vectors are stored in a database layer to enable fast retrieval during analyst interactions.</li>
          </ul>
        </div>
        <ProjectFooter />
      </div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 font-sans px-8 py-12 md:px-24 max-w-7xl mx-auto">
        ...
        <div className="max-w-3xl space-y-8">
          ...
          <p className="text-lg text-slate-400 leading-relaxed">
            Award-winning Web3 multi-tiered game infrastructure built under strict 16-hour hackathon constraints, implementing secure wallet integrations and zero-latency transaction checking structures.
          </p>

          <div className="border-t border-b border-slate-800 py-6 my-8 grid grid-cols-2 md:grid-cols-4 gap-4 font-mono text-xs">
            <div><span className="text-slate-500 block">AWARD</span> Hopper Hacks Winner</div>
            <div><span className="text-slate-500 block">BLOCKCHAIN</span> Solana Devnet</div>
            <div><span className="text-slate-500 block">ENGINE</span> Phaser Top-Down</div>
            <div><span className="text-slate-500 block">BACKEND</span> Django</div>
          </div>

          <h3 className="text-xl font-bold text-slate-200 pt-4">Engineering Achievements</h3>
          <ul className="list-disc list-inside text-slate-400 space-y-2 text-sm leading-relaxed">
            <li>Designed asynchronous transaction layers to process state transitions without blocking the canvas render cycle.</li>
            <li>Configured secure verification channels to authenticate payload validations directly with on-chain ledgers.</li>
            <li>Managed real-time distribution states across active client instances efficiently.</li>
          </ul>
        </div>
        <ProjectFooter />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-32">
        <div className="lg:col-span-8 space-y-8">
          ...
          <p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
            Computer Information Systems student at <span className="text-slate-200 font-semibold">Stony Brook University</span>. Building engineering systems at the convergence of graph structural machine learning, resilient backend architectures, and production scaling mechanics.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/40 border border-slate-800 p-8 rounded hover:border-slate-700 transition-all flex flex-col justify-between group">
            <div>
              ...
              <p className="text-slate-400 text-xs mb-6 leading-relaxed">
                Processes structural relationship graphs using 6-layer GraphSAGE detection networks to uncover complex asset manipulation schemes instantly.
              </p>
            </div>
            <div className="flex flex-wrap gap-1.5 font-mono text-[10px] text-slate-500">
              <span>PyTorch</span>•<span>FastAPI</span>•<span>Gemini</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/40 border border-slate-800 p-8 rounded hover:border-slate-700 transition-all flex flex-col justify-between group">
            <div>
              ...
              <p className="text-slate-400 text-xs mb-6 leading-relaxed">
                Correlates competitive ATP data feeds with real-time eBay pricing components using dedicated Express routing logic.
              </p>
            </div>
            <div className="flex flex-wrap gap-1.5 font-mono text-[10px] text-slate-500">
              <span>React</span>•<span>Node.js</span>•<span>eBay API</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/40 border border-slate-800 p-8 rounded hover:border-slate-700 transition-all flex flex-col justify-between group">
            <div>
              ...
              <p className="text-slate-400 text-xs mb-6 leading-relaxed">
                Real-time Phaser ecosystem processing automated transaction routines over high-throughput cryptographic networks.
              </p>
            </div>
            <div className="flex flex-wrap gap-1.5 font-mono text-[10px] text-slate-500">
              <span>Solana</span>•<span>Django</span>•<span>Vite</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="experience" className="border-t border-slate-800 pt-16 mb-32">
        ...
        <div className="space-y-12 max-w-4xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 relative">
            <div className="md:col-span-4 text-xs font-mono text-slate-500">Jan 2026 – Present</div>
            <div className="md:col-span-8 space-y-2">
              <h3 className="text-base font-bold text-slate-200">Junior ML Systems & Data Pipeline Engineer</h3>
              <div className="text-xs font-mono text-indigo-400">CRIZM — Vertically Integrated Projects Lab</div>
              <p className="text-slate-400 text-xs leading-relaxed pt-2">
                Engineering custom ingestion strategies and multi-format extraction workflows across core grading toolings. Extended Joern-based static analysis components to build robust linting rules processing Python and C++ structures seamlessly within evaluation pipelines.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 relative">
            ...
            <div className="md:col-span-8 space-y-2">
              ...
              <div className="text-xs font-mono text-indigo-400">Auxiliary Services Association, Stony Brook University</div>
              <p className="text-slate-400 text-xs leading-relaxed pt-2">
                Refactoring structural application page indices to match modern WCAG 2.1 Level AA access targets. Managing automated metadata systems and layout architectures for highly visible public web roots.
              </p>
            </div>
          </div>
